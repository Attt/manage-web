layui.use(['element', 'form', 'layer', 'upload'], function () {
    ...
    /* 最大化窗口 */
    $(".my-screen-full").on("click", function (e) {
        e.preventDefault();
            ...
            if (xhr.status == 401) {
                layer.confirm('session连接超时，是否重新登录？', {
                    ...
                    if (window.parent.window != window) {
                        window.top.location = window.location.pathname + '/userLogin/login';
                    }
                });
            }
        }
    ...
    $(".ajax-status").on("click", function (e) {
        e.preventDefault();
        var checked = [];
        var tdcheckbox = $(".my-table td .my-checkbox :checkbox:checked");
    ...
    // 展示数据列表-多选框
    var thcheckbox = $(".my-table th .my-checkbox :checkbox");
    thcheckbox.on("change", function () {
        var tdcheckbox = $(".my-table td .my-checkbox :checkbox");
    ...
    // 检测列表数据是否为空
    var myTable = $(".my-table tbody");
    if (myTable.length > 0) {
        var children = myTable.children();
        if (children.length === 0) {
            var length = $(".my-table thead th").length;
            var trNullInfo = "<tr><td class='my-table-null' colspan='"
                + length + "'>没有找到匹配的记录</td></tr>";
            myTable.append(trNullInfo);
        }
    }
    ...
    var paramSkip = function () {
        var getSearch = "";
        // 搜索框参数
        $('.my-search-box [name]').each(function (key, val) {
